                            ...
                            <select name="{{ $field['name'] }}[]" class="border border-gray-200 rounded p-2 w-full"
                                ...
                                @if (count($games) > 0)
                                    @foreach ($games as $game)
                                        <option value="{{ $game->id }}" {{ $member->games->contains($game->id) ? 'selected' : '' }} name="{{ $game->id }}">
                                            {{ $game->name }}</option>
                                    @endforeach
                                @endif
                            </select>
                            ...
                            <select name="{{ $field['name'] }}" class="border border-gray-200 rounded p-2 w-full" required>
                                @if (count($membershiptypes) > 0)
                                    @foreach ($membershiptypes as $membershiptype)
                                        <option value="{{ $membershiptype->category }}" {{ $member->membership_type->contains($membershiptype->id) ? 'selected' : '' }} >{{ $membershiptype->category }}
                                        </option>
                                    @endforeach
                                @endif
                            </select>
