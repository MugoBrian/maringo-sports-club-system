<x-store>
    <x-card class="p-10 rounded w-full">
        <header>
            <h1 class="text-3xl text-center font-bold my-6 uppercase">
                Manage stock Items
            </h1>
        </header>
        @include('partials._search')
        <div class="mb-4">
            <button class="h-12 w-40 text-white rounded-lg bg-red-500 hover:bg-red-600">
                <a href="/store/stockitems/create" class="text-white"> Add stock Item </a>
            </button>
            <div class="mt-3 p-4 bottom-0 relative">
                {{ $stockItems->links() }}
            </div>
        </div>
        <div class="overflow-x-auto">

            <table class="w-full table-auto rounded-sm">
                <th>Name</th>
                <th>Quantity</th>

                <tbody>
                    @if ($stockItems->count() > 0)
                        @foreach ($stockItems as $stockItem)
                            <tr class="border-gray-300 ">
                                <td class="px-2 py-8 border-t border-b border-gray-300 text-lg ">
                                    {{ $stockItem->sporting_item->name }}
                                    </a>
                                </td>
                                <td class="px-2 py-8 border-t border-b border-gray-300 text-lg">

                                    {{ $stockItem->quantity }}
                                    </a>
                                </td>

                                <td class="px-4 py-8 border-t border-b border-gray-300 text-lg">
                                    <a href="/store/stockitems/{{ $stockItem->id }}/edit"
                                        class="text-blue-400 px-6 py-2 rounded-xl"><i
                                            class="fa-solid fa-pen-to-square"></i>
                                        Edit</a>
                                </td>
                                <td class="px-4 py-8 border-t border-b border-gray-300 text-lg">
                                    <form action="/store/stockitems/{{ $stockItem->id }}/delete" method="POST">
                                        @csrf
                                        @method('delete')
                                        <button class="text-red-600">
                                            <i class="fa-solid fa-trash-can"></i>
                                            Delete
                                        </button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                    @else
                        <tr class="border-gray-300">
                            <td colspan="5"
                                class="px-4 py-8 border-t border-b border-top-width-50 border-gray-300 text-lg">
                                <p class="text-center">No Items Found</p>
                            </td>
                        </tr>
                    @endif
                </tbody>

            </table>
        </div>
    </x-card>
</x-store>
